...
@st.cache_data
def load_td():
    return pd.read_csv("data/anomaly/segment_flags.csv")
...
# --- APP TITLE ---
st.title("Walking Health Summary")
...
with col2:
    stance_status, stance_note = status_and_note(
        patients["mean_stance_pct"],
        lo=55,
        hi=65,
        unit="%",
        lower_note="\n⚠️ Lower stance (standing) time leads to stability reduction or faster steps",
        higher_note="\n⚠️ Higher stance (standing) time indicates that you stood longer before another step and potentially hesitant walking."
    )

    st.metric(
        "**Stance Phase (%)**",
        f"{patients['mean_stance_pct']:.1f} %",
        
    )
    st.markdown(f"{stance_status}")
    st.caption(
        "Normal range: **55–65%** of the walking cycle.\n"
        f"{stance_note}"
    )
...
st.write(
    "This information is for personal awareness only and "
