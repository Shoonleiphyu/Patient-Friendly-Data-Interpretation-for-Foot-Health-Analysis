...
def structural_clean(df: pd.DataFrame, dt_min: float = 0.0, split_gaps: float = 1.0):
    ...
    df['patient_id'] = df['cid']          # preserve original
    df['segment_id'] = df['sub_id']       # new segment id
    df = df.drop(columns=['sub_id'])
    return df

# Savitzky-Golay Filteration
def _clean_records(g: pd.DataFrame, sensors: list[str]) -> pd.DataFrame:
    g = g.sort_values('t').copy()
    g[sensors] = g[sensors].interpolate(limit_direction='both')   # fill gaps
...
def signal_clean(df: pd.DataFrame, sensors: list[str]) -> pd.DataFrame:
...
def remove_outliers(df: pd.DataFrame, cols: list[str], z_thresh: float = 4.0):
    """Z-score filter per column; outliers -> NaN -> interpolate."""
    df = df.copy()
    for c in cols:
...
def normalize_signals(df: pd.DataFrame, cols: list[str]) -> tuple[pd.DataFrame, MinMaxScaler]:
    """MinMax to [0,1] per column. Returns df and the fitted scaler."""
    df = df.copy()
    scaler = MinMaxScaler()
...
def apply_rename(df: pd.DataFrame) -> pd.DataFrame:
    """
    Rename raw pressure and temperature sensor columns.
    """
