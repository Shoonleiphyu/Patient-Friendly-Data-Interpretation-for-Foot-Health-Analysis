...
Z_THRESHOLDS = 3.5  # robust z threshold (MAD-based)
MIN_EVENTS_PER_PATIENT = 6  # baseline segments per patient for within-patient anomalies
...
def _mad(x: np.ndarray) -> float:
    """Median Absolute Deviation, scaled to be comparable to std under normality."""
    x = x[np.isfinite(x)]
    if x.size == 0:
        return np.nan
    med = np.median(x)
